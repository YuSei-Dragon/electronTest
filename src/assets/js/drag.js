...
let vDrag = function (Vue, options = {}) {
	...
	Vue.directive(name, {
		bind (el, binding) {
			let odiv = el
			let disX = 0//鼠标相对元素的位置
			let disY = 0//鼠标相对元素的位置
			let curClientX //鼠标指针坐标
			let curClientY//鼠标指针坐标

			//getAttribute() 方法返回指定属性名的属性值。
			let isFixed = el.getAttribute("fixed") != null

			//getElementsByClassName() 方法返回文档中所有指定类名的元素集合,是一个数组
			let handle = el.getElementsByClassName('v-drag-handle').length > 0 ? el.getElementsByClassName('v-drag-handle')[0] : el

			let zindexEnable = el.getAttribute("zindexEnable")
			if(zindexEnable){
			...
			function mouseMoveHandle (e) {
				let offsetX = curClientX - e.clientX//鼠标与上一次的位置差
				let offsetY = curClientY - e.clientY
				...
				const objRect = odiv.getBoundingClientRect()
				if(left <= 0)left = 0
				if(top <= 0)top = 0
				if(left >= bodyRect.width - objRect.width)left = bodyRect.width - objRect.width
				if(top >= bodyRect.height - objRect.height)top = bodyRect.height - objRect.height
